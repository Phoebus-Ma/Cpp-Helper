...
// needs to be 64 to fully use AMD GPUs
#define WORKGROUP_SIZE 64
// #define PTX
// #define LOG

#ifndef _WIN32
// ignore compiler warnings for CL/cl.hpp with g++
...
struct Device_Info
{
    ...
    inline Device_Info(const cl::Device &cl_device, const cl::Context &cl_context, const uint id)
    {
        ...
        const float nvidia = (float)(contains(to_lower(vendor), "nvidia")) * (nvidia_64_cores_per_cu ? 64.0f : nvidia_192_cores_per_cu ? 192.0f
                                                                                                                                       : 128.0f); // Nvidia GPUs have 192 cores/CU (Kepler), 128 cores/CU (Maxwell, Pascal, Ampere, Hopper, Ada) or 64 cores/CU (P100, Volta, Turing, A100, A30)
        const float amd = (float)(contains_any(to_lower(vendor), {"amd", "advanced"})) * (is_gpu ? (amd_256_cores_per_dualcu ? 256.0f : amd_128_cores_per_dualcu ? 128.0f
                                                                                                                                                                 : 64.0f)
        ...
        // fix wrong global memory reporting for Intel Arc GPUs
        if (intel == 8.0f)
        {
            // 72.5%-87.5% reporting -> /0.8
            if ((contains_any(name, {"A770", "0x56a0"}) && memory >= 11739u && memory < 14168u) || (contains_any(name, {"A770", "A750", "A580", "0x56a0", "0x56a1", "0x56a2"}) && memory >= 5869u && memory < 7084u) || (contains_any(name, {"A380", "0x56a5"}) && memory >= 4402u && memory < 5313u))
            {
                // fix wrong (80% on Windows) memory reporting on Intel Arc
                memory = (uint)((cl_device.getInfo<CL_DEVICE_GLOBAL_MEM_SIZE>() * 5ull / 4ull) / 1048576ull);
            }

            // 87.5%-96.5% reporting -> /0.95
            if ((contains_any(name, {"A770", "0x56a0"}) && memory >= 14168u && memory < 15625u) || (contains_any(name, {"A770", "A750", "A580", "0x56a0", "0x56a1", "0x56a2"}) && memory >= 7084u && memory < 7812u) || (contains_any(name, {"A380", "0x56a5"}) && memory >= 5313u && memory < 5859u))
            {
                // fix wrong (95% on Linux) memory reporting on Intel Arc
                memory = (uint)((cl_device.getInfo<CL_DEVICE_GLOBAL_MEM_SIZE>() * 20ull / 19ull) / 1048576ull);
            }

            // 96.5%-99.0% reporting -> /0.98
            if ((contains_any(name, {"A770", "0x56a0"}) && memory >= 15625u && memory < 16030u) || (contains_any(name, {"A770", "A750", "A580", "0x56a0", "0x56a1", "0x56a2"}) && memory >= 7812u && memory < 8015u) || (contains_any(name, {"A380", "0x56a5"}) && memory >= 5859u && memory < 6011u))
            {
                // fix wrong (98% on Windows) memory reporting on Intel Arc
                memory = (uint)((cl_device.getInfo<CL_DEVICE_GLOBAL_MEM_SIZE>() * 50ull / 49ull) / 1048576ull);
            }
        }
...
inline vector<Device_Info> get_devices(const bool print_info = true)
{
    ...
    for (uint i = 0u; i < (uint)cl_platforms.size(); i++)
    {
        ...
        // same cl::Context for all devices (allocates extra VRAM on all other unused Nvidia GPUs)
        // cl::Context cl_context(cl_devices);

        for (uint j = 0u; j < (uint)cl_devices.size(); j++)
        {
            // separate cl::Context for each device
            cl::Context cl_context(cl_devices[j]);
...
inline Device_Info select_device_with_id(const uint id, const vector<Device_Info> &devices = get_devices())
{
    ...
    else
    {
        ...
        // is never executed, just to avoid compiler warnings.
        return devices[0];
    }
}
...
public:
    ...
    inline Device(const Device_Info &info, const string &opencl_c_code = get_opencl_c_code())
    {
        ...
        // save binary (ptx file).
        write_file("bin/kernel.ptx", cl_program.getInfo<CL_PROGRAM_BINARIES>()[0]);
#endif /* PTX */
    ...
    inline void barrier(const vector<Event> *event_waitlist = nullptr, Event *event_returned = nullptr)
    {
...
private:
    ...
    inline void allocate_device_buffer(Device &device, const bool allocate_device)
    {
        ...
        if (allocate_device)
        {
            ...
            if (error == -61)
                print_error("Memory size is too large at " + to_string((uint)(capacity() / 1048576ull)) + " MB. Device \"" + device.info.name + "\" accepts a maximum buffer size of " + to_string(device.info.max_global_buffer) + " MB.");
            else if (error)
...
public:
    ...
    inline Memory &operator=(Memory &&memory) noexcept
    {
        ...
        {
            // transfer host_buffer pointer.
            host_buffer = memory.exchange_host_buffer(nullptr);
            initialize_auxiliary_pointers();
            host_buffer_exists = true;
        }

        // destructor of memory will be called automatically.
        return *this;
    }
    ...
    {
        // faster than "write_to_device();"
        // if(device_buffer_exists) cl_queue.enqueueFillBuffer(device_buffer, value, 0ull, capacity());

        // faster than "for(ulong i=0ull; i<range(); i++) host_buffer[i] = value;"
        if (host_buffer_exists)
            std::fill(host_buffer, host_buffer + range(), value);

        // enqueueFillBuffer is broken for large buffers on Nvidia GPUs!
        write_to_device();
    ...
    inline void write_to_device(const ulong offset, const ulong length, const bool blocking = true, const vector<Event> *event_waitlist = nullptr, Event *event_returned = nullptr)
    {
        if (host_buffer_exists && device_buffer_exists)
        {
            ...
            if (safe_length > 0ull)
                cl_queue.enqueueWriteBuffer(device_buffer, blocking, safe_offset * sizeof(T), safe_length * sizeof(T), (void *)(host_buffer + safe_offset), event_waitlist, event_returned);
        }
    }

    // read 1D domain from device, either for all vector dimensions (-1) or for a specified dimension.
    inline void read_from_device_1d(const ulong x0, const ulong x1, const int dimension = -1, const bool blocking = true, const vector<Event> *event_waitlist = nullptr, Event *event_returned = nullptr)
    {
        if (host_buffer_exists && device_buffer_exists)
        {
            ...
            for (uint i = i0; i < i1; i++)
            {
                ...
                if (safe_length > 0ull)
                    cl_queue.enqueueReadBuffer(device_buffer, false, safe_offset * sizeof(T), safe_length * sizeof(T), (void *)(host_buffer + safe_offset), event_waitlist, event_returned);
            }
    ...
    inline void write_to_device_1d(const ulong x0, const ulong x1, const int dimension = -1, const bool blocking = true, const vector<Event> *event_waitlist = nullptr, Event *event_returned = nullptr)
    {
        if (host_buffer_exists && device_buffer_exists)
        {
            const uint i0 = (uint)max(0, dimension), i1 = dimension < 0 ? d : i0 + 1u;
            ...
            for (uint i = i0; i < i1; i++)
            {
                ...
                if (safe_length > 0ull)
                    cl_queue.enqueueWriteBuffer(device_buffer, false, safe_offset * sizeof(T), safe_length * sizeof(T), (void *)(host_buffer + safe_offset), event_waitlist, event_returned);
            }
    ...
    inline void read_from_device_2d(const ulong x0, const ulong x1, const ulong y0, const ulong y1, const ulong Nx, const ulong Ny, const int dimension = -1, const bool blocking = true, const vector<Event> *event_waitlist = nullptr, Event *event_returned = nullptr)
    {
        if (host_buffer_exists && device_buffer_exists)
        {
    ...
    inline void write_to_device_2d(const ulong x0, const ulong x1, const ulong y0, const ulong y1, const ulong Nx, const ulong Ny, const int dimension = -1, const bool blocking = true, const vector<Event> *event_waitlist = nullptr, Event *event_returned = nullptr)
    {
        if (host_buffer_exists && device_buffer_exists)
        {
            for (uint y = y0; y < y1; y++)
    ...
    inline void read_from_device_3d(const ulong x0, const ulong x1, const ulong y0, const ulong y1, const ulong z0, const ulong z1, const ulong Nx, const ulong Ny, const ulong Nz, const int dimension = -1, const bool blocking = true, const vector<Event> *event_waitlist = nullptr, Event *event_returned = nullptr)
    {
        if (host_buffer_exists && device_buffer_exists)
        {
    ...
    inline void write_to_device_3d(const ulong x0, const ulong x1, const ulong y0, const ulong y1, const ulong z0, const ulong z1, const ulong Nx, const ulong Ny, const ulong Nz, const int dimension = -1, const bool blocking = true, const vector<Event> *event_waitlist = nullptr, Event *event_returned = nullptr)
    {
        if (host_buffer_exists && device_buffer_exists)
        {
            for (uint z = z0; z < z1; z++)
...
private:
    ...
    {
        if (error == -48)
            print_error("There is no OpenCL kernel with name \"" + name + "(...)\" in the OpenCL C code! Check spelling!");

        if (error < -48 && error > -53)
            print_error("Parameters for OpenCL kernel \"" + name + "(...)\" don't match between C++ and OpenCL C!");

        if (error == -54)
            print_error("Workgrop size " + to_string(WORKGROUP_SIZE) + " for OpenCL kernel \"" + name + "(...)\" is invalid!");

        if (error != 0)
            print_error("OpenCL kernel \"" + name + "(...)\" failed with error code " + to_string(error) + "!");
    }
...
public:
    ...
    template <class... T>
    inline Kernel(const Device &device, const ulong N, const string &name, const T &...parameters)
    {
        if (!device.is_initialized())
        ...
        // expand variadic template to link kernel parameters.
    ...
    // accepts Memory<T> objects and fundamental data type constants.
    template <class... T>
    inline Kernel(const Device &device, const ulong N, const uint workgroup_size, const string &name, const T &...parameters)
    {
        if (!device.is_initialized())
            print_error("No OpenCL Device selected. Call Device constructor.");
