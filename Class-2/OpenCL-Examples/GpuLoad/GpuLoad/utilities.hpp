...
inline string to_string(double x, const uint decimals)
{
    ...
    return s + to_string(integral) +
           (decimals == 0u ? "" : "." + decimal_to_string_double(decimal, min((int)decimals, 16)));
}
...
inline bool is_number(const string &s)
{
    return equals_regex(s, "\\d+(u|l|ul|ll|ull)?") ||
           ...
           equals_regex(s, "(((\\d+\\.?\\d*|\\.\\d+)([eE][+-]?\\d+[fF]?)?)|(\\d+\\.\\d*|\\.\\d+)[fF]?)");
}

// print formatted message.
inline void print_message(const string &message, const string &keyword = "", const int keyword_color = -1, const int colons = true)
{
    const uint k = length(keyword) + 2u, w = CONSOLE_WIDTH - 4u - k;
    string p = colons ? ": " : "  ", f = "";

    // length of current line of words.
    uint l = 0u;
    ...
    for (uint i = 0u; i < (uint)v.size(); i++)
    {
        ...
        // word fits -> append word and space.
        if (l <= w + 1u)
        {
            p += word + " ";
        }
        // word overflows -> split word into next line
        else if (wordlength > w)
        {
            p += substring(word, 0, w - (l - wordlength - 1u)) + " |\n| " + f;
            v[i] = substring(v[i], w - (l - wordlength - 1u));
            i--;    // reuse same vector element for overflowing part, decrement i to start next line with this overflowing part
            l = 0u; // reset line length
        }
        // word does not fit -> fill remaining line with spaces
        else
        {
...
#else // UTILITIES_REGEX
// print message
inline void print_message(const string &message, const string &keyword = "", const int colons = true)
{
...
// usage: set_environment_variable((char*)"VARIABLE=VALUE");
inline void set_environment_variable(char *s)
{
#if defined(_WIN32)
    (void)_putenv(s);
...
{
    // remove existing file extension if existing and replace it with new one
    return filename.substr(0, filename.rfind('.')) + (extension.at(0) != '.' ? "." : "") + extension;
}
